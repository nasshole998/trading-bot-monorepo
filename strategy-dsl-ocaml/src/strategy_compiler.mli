...
(** Type for a compiled strategy. It's a function that takes the current execution environment,
    ...
    the updated execution environment and a list of actions to perform, wrapped in Result and Lwt for error handling. *)
type compiled_strategy = exec_env -> Strategy_data.t -> Strategy_grpc.t -> string -> (exec_env * Strategy_ast.action list, [> `ExecutionError of string]) result Lwt.t

(** Exception raised during execution errors (used internally before returning Result). *)
...
val execute_compiled_strategy : compiled_strategy -> Strategy_data.t -> Strategy_grpc.t -> string -> exec_env -> (exec_env * Strategy_ast.action list, [> `ExecutionError of string]) result Lwt.t